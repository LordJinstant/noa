...
function applyModeratorView() {
  ...
  if (label) {
    label.textContent = role ? ('Signed in as ' + role.replace('_', ' ')) : '';
  }
  // Moderators: AI messages / tickets only — no user or admin management
  if (role === 'moderator') {
    const hideIds = ['navManageUsers', 'createAdminCard', 'aiTrainNav'];
    hideIds.forEach(function (id) {
      const el = document.getElementById(id);
      if (el) el.classList.add('hidden');
    });
    // Hide any heading that says Create New Admin
    document.querySelectorAll('h2, h3, h4').forEach(function (h) {
      const t = (h.textContent || '').toLowerCase();
      if (t.includes('create') && t.includes('admin')) {
        const card = h.closest('div.bg-white, section, article') || h.parentElement;
        if (card) card.classList.add('hidden');
      }
    });
  }
  // Train AI still super_admin only
  if (role === 'super_admin') {
    const nav = document.getElementById('aiTrainNav');
    if (nav) nav.classList.remove('hidden');
  }
